...
- **补全规避**（v1.5.0）—— 面向带代码补全的在线编辑器（如学习通编程题）：开启后在空格/回车/Tab 前自动注入 `Esc` 关闭补全弹窗，防止这三个键被解释为"接受候选"而把代码打乱
...
4. 往带代码补全的编辑器（在线编程作业等）键入代码时，勾选“补全规避”，在空格/回车/Tab 前自动关闭补全弹窗
5. 调节倒计时秒数
6. 点击 **启动**
...
- **补全规避的边界** —— 该开关以 `Esc` 关闭目标编辑器的补全弹窗，覆盖空格/回车/Tab 被劫持为"接受候选"的常见情形；但弹窗无法从外部探测，无弹窗时也会注入 `Esc` —— 目标处于浏览器全屏（Esc 会退出全屏）或 Vim 等 Esc 是功能键的场景请勿开启。括号/符号的自动配对不受此开关影响（主流编辑器对重复输入的右括号有跳过处理）。
...
`testdata/completion-guard.html` 复刻"禁粘贴 + 关键词候选补全"的在线编程编辑器（学习通作业一类）：输入字母即弹候选，弹窗开着时回车/空格/Tab 是否"接受候选"由页内开关决定，`Esc` 关闭弹窗；内置逐键事件日志与预期文本对比，用于验证"补全规避"开关的效果，并定位目标编辑器究竟劫持了哪个键。用法详见文件头部注释。
