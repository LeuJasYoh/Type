...
declare global {
  interface Window {
    /** 启动输入: text 内容, delay 倒计时秒数, forceRaw 是否绕过剪贴板降级, avoidCompletion 是否补全规避 */
    startTyping(text: string, delay: number, forceRaw: boolean, avoidCompletion: boolean): Promise<string>;
    /** 取消当前输入 */
...
export const startTyping = (text: string, delay: number, forceRaw: boolean, avoidCompletion: boolean) =>
  window.startTyping(text, delay, forceRaw, avoidCompletion);
